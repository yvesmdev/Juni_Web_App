    ...
    public class DatabaseRepository
    {
        ...
        public static List<AgentApplication> GetAgentApplicationList()
        {
            ...
            using (MySqlConnection DbCon = new MySqlConnection(ConnectionString))
            {
                ...
                while (DbReader.Read())
                {
                    ...
                    CurApplication.CountryCode = DbReader["country_code"] as string ?? CurApplication.CountryCode;
        ...
        public static bool AddApplication(AgentApplication CurApplication)
        {
            ...
            using (MySqlConnection DbCon = new MySqlConnection(ConnectionString))
            {
                ...
                string Query = "INSERT INTO agent_application(tel,country_code,full_name,email,street_address,suburb,municipality,city,province,application_date,application_approved)" +
                    " VALUES(@tel,@countryCode,@fullName,@email,@streetAddress,@suburb,@municipality,@city,@province,@applicationDate,@applicationStatus); SELECT LAST_INSERT_ID()";

                MySqlCommand DbCommand = new MySqlCommand(Query, DbCon);
                DbCommand.Parameters.AddWithValue("@tel", CurApplication.CellNumber);
                DbCommand.Parameters.AddWithValue("@countryCode", CurApplication.CountryCode);//.CellNumber);
        ...
        public static int DismissAgent(string agent_id, string reason)
        {
            ...
            using (MySqlConnection DbCon2 = new MySqlConnection(ConnectionString))
            {
                ...
                using (MySqlConnection DbCon3 = new MySqlConnection(ConnectionString))
                {
                    ...
                    SendEmailInBackground(new string[] { CurUser.email}, "Juni - Retrait d'Agent", messageClientEmailApp);
                    //Approve application
                   /* Query = "UPDATE agent_application SET application_approved=-1 WHERE id=" + CurApplication.id;
                    MySqlCommand DbCommand4 = new MySqlCommand(Query, DbCon3);
                    DbCommand4.ExecuteNonQuery();*/
        ...
        public static CouponProfile GetCouponProfileById(string couponCode)
        {
            ...
            {
                DbCon.Open();
                DbCommand = new MySqlCommand("SELECT * FROM user_profile WHERE (coupon_code='" + couponCode + "') AND agent_approved=1 ", DbCon);
